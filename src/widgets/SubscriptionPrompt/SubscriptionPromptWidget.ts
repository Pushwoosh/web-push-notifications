...
import { getSubscriptionPromptSkipReason, getWidgetConfig, updateCappingParams } from './helpers';
...
export class PWSubscriptionPromptWidget {
  ...
  public async run(): Promise<void> {
    ...
    // Nothing to ask for when push is unavailable (the npm export can be
    // constructed directly, without the entry point's own check).
    if (!pw.isPushAvailable()) {
      return;
    }
    ...
    // Same check the entry point runs before fetching this bundle; kept here
    // too, the bundle can also be loaded directly (npm export, own loader).
    if (await getSubscriptionPromptSkipReason(features, pw, widgetConfig)) {
      return;
    }
    ...
    // `autoSubscribe: false` on this use case is already a skip reason above.
    const isNotSetUseCase = currentPromptUseCase === SUBSCRIPTION_WIDGET_USE_CASE_NOT_SET;
