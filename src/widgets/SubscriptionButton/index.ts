...
export class PWSubscriptionButtonWidget {
  ...
  public async run() {
    // Nothing to subscribe to when push is unavailable, and the constructor
    // leaves the config unset in that case — so keep this before any render.
    if (!this.pw.isPushAvailable()) {
      return;
    }
