...
export class PWInboxWidget {
  ...
  /** Visible slide per carousel message, kept across the widget's re-renders. */
  activeSlides: { [code: string]: number };
  ...
  private initTrigger() {
    ...
    // Init has just pulled the inbox itself, so the first bell open must not
    // fetch the whole thing again seconds later.
  ...
  private renderWidget() {
    ...
    this.widget.classList.toggle('pw-inbox-widget--legacy-cell', this.config.legacyClassicCell);
  ...
  private getStyle(): HTMLStyleElement {
    ...
    styleNode.innerHTML = inboxWidgetStyleCss.toString();
    return styleNode;
  }

  private applyStyleVariables() {
    applyStyleVariables(this.widget, STYLE_PREFIX, CONFIG_STYLES, this.config);
    applyStyleVariables(this.trigger, STYLE_PREFIX, TRIGGER_CONFIG_STYLES, this.config);
  }
  ...
  private renderMessages() {
    ...
    // innerHTML was just replaced, so every element collected last render is
    // detached: keeping them leaks and feeds stale offsets to the viewport check.
    this.messagesElements = {};
    ...
    this.messages.forEach((message: IInboxMessagePublic) => {
      ...
      messageElement.innerHTML = messageTemplate(message, this.activeSlides[message.code] || 0);
  ...
  private updateReadStatus() {
    // Taken and cleared in one step: the codes are reported once, and a long
    // session does not keep growing the array across every open/close cycle.
    const codes = this.readItems;
    this.readItems = [];

    this.pw.pwinbox.readMessagesWithCodes(codes).then(this.updateInbox);
  }

  // Opening the bell is the only refresh besides init: a standalone App Inbox
  // message rides a channel with no sender, so no push announces one.
  private syncMessages() {
    // getInboxMessages returns the whole inbox, with no paging on the server
    // side, so toggling the bell must not turn into a request per click.
    if (Date.now() - this.lastSyncAt < SYNC_INTERVAL) {
      return;
    }

    this.lastSyncAt = Date.now();

    this.pw.pwinbox.syncMessages().catch((error: unknown) => {
      // Not the SDK Logger: it drags `core/storage` in and costs this bundle
      // ~20 KiB. SubscriptionButton logs the same way for the same reason.
      console.error('Pushwoosh: cannot refresh the inbox on open', error);
    });
  }
  ...
  private performMessageAction(code: string) {
    const message = this.messages.find((item: IInboxMessagePublic): boolean => item.code === code);

    // The popup is modal and locks body scroll, so the panel it was opened
    // from is left stranded under the overlay.
    if (message && message.webPopupCode) {
      this.toggle(false);
    }
  ...
  private removeMessages(messages: Array<string>) {
    // Was `slice(indexOf(code), 1)`, which returns an empty array for any index
    // past 1 and so discarded every pending read code, not just this one.
    this.readItems = this.readItems.filter((code: string) => messages.indexOf(code) === -1);
  ...
  private onWidgetClickHandler(event: MouseEvent) {
    ...
    // A pager dot only switches the slide: it must not open the message.
    const dotElement = (<HTMLElement>event.target).closest('.pw-inbox_dot');

    if (dotElement) {
      this.showSlide(messageCode, parseInt(dotElement.getAttribute('data-pw-slide') || '0', 10));
      return;
    }

    // A slide link goes there instead of the whole message's action, as in the
    // iOS SDK. Re-checked here because the value round-trips through the DOM.
    const slideElement = (<HTMLElement>event.target).closest('.pw-inbox_slide');
    const slideLink = slideElement && slideElement.getAttribute('data-pw-slide-link');

    if (slideLink && isSafeUrl(slideLink)) {
      document.location.href = slideLink;
      return;
    }
  ...
  /** Switch the visible slide of one carousel message and redraw just that cell. */
  private showSlide(code: string, index: number) {
    this.activeSlides[code] = index;

    const messageElement = this.messagesElements[code];
    const message = this.messages.find((item: IInboxMessagePublic): boolean => item.code === code);

    if (messageElement && message) {
      messageElement.innerHTML = messageTemplate(message, index);
    }
  }
