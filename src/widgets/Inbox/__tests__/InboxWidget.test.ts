...
describe('PWInboxWidget', () => {
  ...
  it('closes the panel when the message opens a web popup', async () => {
    const message = { ...buildCarouselMessage(), layout: <const>'classic', carousel: [], webPopupCode: 'AAAAA-BBBBB' };
    const { widget, pwinbox } = buildWidget([message]);
    await widget.run();
    await Promise.resolve();
    await Promise.resolve();
    ...
    (<HTMLElement>document.querySelector('.pw-inbox_item')).dispatchEvent(new MouseEvent('click', { bubbles: true }));

    expect(pwinbox.performActionForMessageWithCode).toHaveBeenCalledWith('abc123');
    expect(widget.isOpened).toBe(false);
  });
