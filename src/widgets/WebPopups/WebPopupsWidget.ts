import { collectWebPopupFormCodes, type WebPopupParams } from 'smart-blocks-utils';

import { POPUP_HOST_Z_INDEX, SUBSCRIPTION_FORMS_WIDGET_TAG, WEB_POPUPS_LOG_PREFIX, WEB_POPUPS_WIDGET_NAMESPACE } from './constants';
...
import { type PageRule, type WebPopup } from './types';
...
export class WebPopupsWidget {
  ...
  public async run(): Promise<void> {
    ...
    results.forEach((result, index) => {
      if (result.status === 'rejected') {
        Logger.error(result.reason, `${WEB_POPUPS_LOG_PREFIX} Failed to load web popup "${webPopups[index].code}"`);
      }
    });

    Logger.debug(
      `${WEB_POPUPS_LOG_PREFIX} ${this.loadedPopups.size} popup(s) passed strong conditions and loaded:`,
      Array.from(this.loadedPopups.keys()),
    );
  ...
  private renderForCurrentPage(): void {
    ...
    Logger.debug(
      `${WEB_POPUPS_LOG_PREFIX} ${toRender.length} popup(s) selected for display on "${window.location.pathname}":`,
      toRender.map(([code]) => code),
    );
  ...
  private addToQueue(loaded: LoadedWebPopup): void {
    ...
    Logger.debug(`${WEB_POPUPS_LOG_PREFIX} Popup "${webPopup.code}" queued for display in ${webPopup.delay}s`);

    this.pw.api.recordPopupFormEvent(webPopup.code, 1).catch((error) => {
      Logger.error(error, `${WEB_POPUPS_LOG_PREFIX} Failed to record web popup queued event for "${webPopup.code}"`);
    });
    ...
    const timeoutId = window.setTimeout(() => {
      ...
      if (loaded.kind === 'subscription-form') {
        ...
        const formCodes = collectWebPopupFormCodes(loaded.parsed);
        if (formCodes.length > 0) {
          this.pw.data.getHwid()
            .then((hwid) => formCodes.forEach((formCode) => recordSubscriptionFormView(formCode, hwid)))
            .catch((error) => {
              Logger.error(error, `${WEB_POPUPS_LOG_PREFIX} Failed to record subscription form views for web popup "${webPopup.code}"`);
            });
        }
      }

      Logger.debug(`${WEB_POPUPS_LOG_PREFIX} Popup "${webPopup.code}" shown (kind: ${loaded.kind})`);

      this.markAsShown(webPopup).catch((error) => {
        Logger.error(error, `${WEB_POPUPS_LOG_PREFIX} Failed to mark web popup "${webPopup.code}" as shown`);
      });
