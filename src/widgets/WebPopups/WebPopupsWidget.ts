...
import { POPUP_HOST_Z_INDEX, WEB_POPUPS_WIDGET_NAMESPACE } from './constants';
...
export class WebPopupsWidget {
  ...
  private async loadWebPopup(webPopup: WebPopup): Promise<void> {
    ...
    this.loadedPopups.set(webPopup.code, { popup: webPopup, parsed });
  }
  ...
  private renderForCurrentPage(): void {
    ...
    toRender.forEach(([code, { popup, parsed }]) => {
      this.addToQueue(popup, parsed);
  ...
  private addToQueue(webPopup: WebPopup, parsed: WebPopupParams): void {
    ...
    const timeoutId = window.setTimeout(() => {
      ...
      renderWebPopup(container, parsed, closePopup);
