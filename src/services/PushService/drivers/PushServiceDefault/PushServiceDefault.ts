...
export class PushServiceDefault implements IPushService {
  ...
  public async checkIsNeedResubscribe(): Promise<boolean> {
    ...
    if (lastPermission !== permission) {
      ...
      // A first visit has no recorded permission, so nothing has changed yet;
      // the token comparison below still catches a subscription out of sync.
      if (lastPermission !== undefined) {
        return true;
      }
    }
