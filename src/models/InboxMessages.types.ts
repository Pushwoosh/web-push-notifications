import { type IInboxMessageSlide, type TInboxLayout } from './InboxCell.types';
...
export interface IInboxMessageActionParams {
  ...
  h?: string;
  r?: string;
  /** Hero image of the banner and captioned layouts (the campaign's `bannerUrl`). */
  b?: string;
  /** Cell contract and custom data, merged flat. Object or JSON string; see `parseUserData`. */
  u?: unknown;
  /** Web popup opened by a tap. The web counterpart of `rm`: an entry carries one or the other. */
  wp?: string;
}
...
export interface IInboxMessagePublic {
  ...
  /** Cell layout to draw; already degraded to one this message can render. */
  layout: TInboxLayout;
  /** Small round avatar. Same value as `imageUrl`, under the name the layouts use. */
  iconUrl: string;
  /** Hero image of the banner and captioned layouts; empty when none was sent. */
  heroUrl: string;
  /** Slides of the carousel layout; empty for every other layout. */
  carousel: Array<IInboxMessageSlide>;
  /** The campaign's own custom data, as sent from the control panel. */
  customData: Record<string, unknown>;
  /** Web popup a tap opens; empty when the message opens `link` instead. */
  webPopupCode: string;
}
...
export interface IInboxMessages {
  ...
  syncMessages(): Promise<void>; // pull the inbox from the server
