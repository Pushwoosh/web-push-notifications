...
import { type WebPopup } from '../../widgets/WebPopups/types';
...
export class Api {
  ...
  // Read from init params, not from the store: a store that keeps nothing is
  // exactly when the userId is the only identity left.
  private readonly getInitUserId: () => string | undefined;
  private ensureDeviceRegisteredPromise: Promise<boolean> | null = null;
  private checkDevicePromise: Promise<IMapResponse['checkDevice']> | null = null;
  ...
  constructor(
    ...
    getInitUserId: () => string | undefined = () => undefined,
  ) {
    this.eventBus = eventBus;
    this.data = data;
    this.apiClient = apiClient;
    this.getIsCommunicationDisabled = getIsCommunicationDisabled;
    this.getInitUserId = getInitUserId;
  }

  // Memoised per page like ensureDeviceRegistered: init alone asked for the same
  // device twice. Every call that changes the device on the server drops it.
  public async checkDevice(): Promise<IMapResponse['checkDevice']> {
    if (!this.checkDevicePromise) {
      this.checkDevicePromise = this.requestCheckDevice()
        .catch((error) => {
          this.checkDevicePromise = null;
          throw error;
        });
    }

    return this.checkDevicePromise;
  }

  private async requestCheckDevice(): Promise<IMapResponse['checkDevice']> {
  ...
  // Memoised per page: inbox and web popups both need the device, and each
  // extra call would cost another checkDevice request. True = created here.
  public async ensureDeviceRegistered(): Promise<boolean> {
    if (this.getIsCommunicationDisabled()) {
      return false;
    }

    if (!this.ensureDeviceRegisteredPromise) {
      this.ensureDeviceRegisteredPromise = this.registerDeviceIfNotExist()
        .catch((error) => {
          this.ensureDeviceRegisteredPromise = null;
          throw error;
        });
    }

    return this.ensureDeviceRegisteredPromise;
  }

  private async registerDeviceIfNotExist(): Promise<boolean> {
    const { exist } = await this.checkDevice();

    if (exist) {
      return false;
    }

    await this.apiClient.registerDevice(await this.getRequestParams());
    this.checkDevicePromise = null;
  ...
  public async setLanguage(language: string): Promise<IMapResponse['setTags']> {
    if (this.getIsCommunicationDisabled()) {
      return Promise.reject(new Error('Communication is disabled'));
    }

    await this.data.setLanguage(language);
    await this.data.setStatusLanguageWasChanged(true);

    return this.setTags({ Language: language });
  }
  ...
  public async registerEmail(email: string, isManual = true): Promise<IMapResponse['registerEmail']> {
    ...
    const requestParams = await this.getIdentityRequestParams();
  ...
  public async registerSmsNumber(number: string, isManual = true): Promise<IMapResponse['registerSmsNumber']> {
    ...
    const { application, userId } = await this.getIdentityRequestParams();
  ...
  public async getPopupForms(): Promise<Array<WebPopup>> {
    if (this.getIsCommunicationDisabled()) {
      return [];
    }

    const response = await this.apiClient.getPopupForms(await this.getRequestParams());

    return response.popup_forms ?? [];
  }

  public async getPopupFormContent(code: string): Promise<IMapResponse['getPopupFormContent']> {
    if (this.getIsCommunicationDisabled()) {
      return Promise.reject(new Error('Communication is disabled'));
    }

    return this.apiClient.getPopupFormContent({ ...await this.getRequestParams(), code });
  }

  public async recordPopupFormEvent(code: string, event: 1 | 2): Promise<IMapResponse['recordPopupFormEvent']> {
    if (this.getIsCommunicationDisabled()) {
      return Promise.reject(new Error('Communication is disabled'));
    }

    return this.apiClient.recordPopupFormEvent({
      ...await this.getRequestParams(),
      code,
      event,
    });
  ...
  public async multiRegisterDevice(request: Partial<MultiRegisterDeviceRequest>): Promise<IMapResponse['multiRegisterDevice']> {
    ...
    // The push device is built from the hwid at the end of this method, by when
    // the local store already says the channels were registered.
    if (!request.push_devices && tokens.pushToken) {
      await this.requireHwid();
    }
  ...
  // Single door for every hwid-bound request: the backend rejects a request
  // with no hwid, so it must not leave the SDK in the first place.
  private async getRequestParams(): Promise<IRequest> {
    const hwid = await this.requireHwid();
    const deviceType = await this.data.getDeviceType();
    const params = await this.getIdentityRequestParams();
  ...
  // Email, SMS and WhatsApp are registered against the user, not the device, so
  // they take the same params without the hwid the store may not have.
  private async getIdentityRequestParams(): Promise<Omit<IRequest, 'hwid' | 'device_type'>> {
    const applicationCode = await this.data.getApplicationCode();
    const hwid = await this.data.getHwid();
    const userId = await this.data.getUserId() || this.getInitUserId();
    const deviceModel = await this.data.getDeviceModel();
    const language = await this.data.getLanguage();
    const version = await this.data.getSdkVersion();
    ...
    // Both empty means the request identifies nobody and the backend rejects it.
    if (!userId && !hwid) {
      throw new Error(CONSTANTS.IDENTITY_UNAVAILABLE_MESSAGE);
    }
  ...
  private async requireHwid(): Promise<string> {
    const hwid = await this.data.getHwid();

    if (!hwid) {
      throw new Error(CONSTANTS.HWID_UNAVAILABLE_MESSAGE);
    }
