...
export class Data {
  ...
  public async getShownPopupForms(): Promise<string[]> {
    return this.store.get(getKey('shownPopupForms'), []);
  }

  public async addShownPopupForm(code: string): Promise<void> {
    const shown = await this.getShownPopupForms();
    if (shown.includes(code)) {
      return;
    }

    shown.push(code);
    await this.store.set(getKey('shownPopupForms'), shown);
  }
