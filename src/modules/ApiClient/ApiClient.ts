import { ApiRequestError } from './ApiClient.errors';
...
export class ApiClient {
  ...
  public getPopupForms(options: IMapRequest['getPopupForms']): Promise<IMapResponse['getPopupForms']> {
    return this.createRequest('getPopupForms', options);
  }

  public getPopupFormContent(options: IMapRequest['getPopupFormContent']): Promise<IMapResponse['getPopupFormContent']> {
    return this.createRequest('getPopupFormContent', options);
  }

  public recordPopupFormEvent(options: IMapRequest['recordPopupFormEvent']): Promise<IMapResponse['recordPopupFormEvent']> {
    return this.createRequest('recordPopupFormEvent', options);
  }
  ...
  private async createRequest<T extends TMethod>(
    ...
    const url = entrypoint + methodName;
  ...
  private async checkResponse(response: Response): Promise<any> {
    if (response.status !== 200) {
      ...
      try {
        ...
        // An application-level code arrives here too: the relaxed API answers
        // a rejected application code with HTTP 400 and status_code 210.
      ...
      throw new ApiRequestError(errorMessage, statusCode);
    }
    ...
    if (data.status_code !== 200) {
      throw new ApiRequestError(
        `Error code: ${data.status_code}. Error text: ${data.status_message}`,
