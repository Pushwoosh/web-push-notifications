import { type IServerInitParams } from '../../core/resolveInitParams';
import { type IInboxMessage } from '../../models/InboxMessages.types';
import { type WebPopup } from '../../widgets/WebPopups/types';
import { type MultiRegisterDeviceRequest } from '../Api/Api.types';
...
type TMethodGetPopupForms = 'getPopupForms';
type TMethodGetPopupFormContent = 'getPopupFormContent';
type TMethodRecordPopupFormEvent = 'recordPopupFormEvent';
...
type TMethodMultiRegisterDevice = 'multiRegisterDevice';
...
interface IRequestGetPopupFormContent extends IRequest {
  code: string; // popup form content code
}

interface IRequestRecordPopupFormEvent extends IRequest {
  code: string; // popup form content code
  event: 1 | 2; // 1 - VISIT, 2 - IMPRESSION
}
...
interface IResponseGetConfig {
  features: {
    ...
    events?: string[];
    subscription_prompt?: {
      use_case: 'not-set' | 'default' | 'not-used';
    };
    web_sdk?: IServerInitParams;
  };
}

interface IResponseGetPopupForms {
  popup_forms: Array<WebPopup>;
}

interface IResponseGetPopupFormContent {
  code: string;
  json: string;
  html: string | null;
}
...
export type TMethod =
  ...
  | TMethodGetPopupForms
  | TMethodGetPopupFormContent
  | TMethodRecordPopupFormEvent
  | TMethodSetEmailTags
  | TMethodPostEvent
  | TMethodGetInboxMessages
  | TMethodInboxStatus
  | TMethodMultiRegisterDevice;
...
export interface IMapRequest {
  ...
  getPopupForms: IRequest;
  getPopupFormContent: IRequestGetPopupFormContent;
  recordPopupFormEvent: IRequestRecordPopupFormEvent;
  ...
  multiRegisterDevice: MultiRegisterDeviceRequest;
}
...
export interface IMapResponse {
  ...
  getPopupForms: IResponseGetPopupForms;
  getPopupFormContent: IResponseGetPopupFormContent;
  recordPopupFormEvent: IResponse;
  setEmailTags: IResponse;
  postEvent: IResponsePostEvent;
  getInboxMessages: IResponseGetInboxMessages;
  inboxStatus: IResponse;
  multiRegisterDevice: IResponse;
}
