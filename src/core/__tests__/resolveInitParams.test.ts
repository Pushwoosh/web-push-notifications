...
describe('resolveInitParams', () => {
  ...
  it('takes unifiedPlatform from the panel', () => {
    expect(resolveInitParams(raw(), { unifiedPlatform: true })).toEqual({ unifiedPlatform: true });
  });

  it('lets the panel turn unifiedPlatform off for a snippet that enabled it', () => {
    expect(resolveInitParams(raw({ unifiedPlatform: true }), { unifiedPlatform: false }))
      .toEqual({ unifiedPlatform: false });
  });
