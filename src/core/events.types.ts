import { type WebPopupHideReason, type WebPopupShowTrigger } from '../widgets/WebPopups/types';
...
export interface EventHandlerMap {
  ...
  /* web popups events */
  // Fired once the widget has loaded the popup list — the point from which
  // Pushwoosh.moduleRegistry.webPopups.show() acts immediately.
  'web-popups-ready': EventHandler<EventPayload<Record<string, never>>>;
  // `trigger` tells an automatic display from a show() call.
  'show-web-popup': EventHandler<EventPayload<{ code: string; trigger: WebPopupShowTrigger }>>;
  // `reason` distinguishes a user dismissal from hide() and from a popup being
  // closed to make room for a show() call.
  'hide-web-popup': EventHandler<EventPayload<{ code: string; reason: WebPopupHideReason }>>;
}
