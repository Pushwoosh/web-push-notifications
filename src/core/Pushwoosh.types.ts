...
import { type IWebPopupsConfig, type IWebPopupsPublicApi } from '../widgets/WebPopups/types';

/**
 * Widgets publish their programmatic interfaces here, keyed by widget name, so
 * a site can drive them from its own code. Every entry is optional, and when it
 * appears differs per widget: `webPopups` registers as soon as its bundle
 * loads, while `subscribePopup` and `subscriptionPrompt` register from inside
 * their run() and so stay absent when it returns early (already subscribed,
 * permission already decided).
 */
export interface IModuleRegistry {
  webPopups?: IWebPopupsPublicApi;
  subscribePopup?: {
    show(): void;
    hide(): void;
  ...
  // Widgets are registered dynamically and some consumers read them untyped.
  [key: string]: any;
}
