...
// Configs
const defaultConfig = require(path.resolve(__dirname, 'config.mjs')).default; // Default
...
const LOCAL_CONFIG_PATH = path.resolve(__dirname, 'config.local.mjs');
const localConfig = fs.existsSync(LOCAL_CONFIG_PATH) && process.env.NODE_ENV !== 'production'
  ? require(LOCAL_CONFIG_PATH).default
  : {}; // Local
